...
import calculateAverageStarRate from './calculateAverageStarRate';
...
export default async function createReviewListData (resId: number, email: string ,starRate: number, rContent: string, inputImage?: File)  {
    try{
        ...
        const reviewQuery = query(collection(ChungstaurantFirestore, 'ReviewListData'), where("restaurantId", "==", resIdNum));
        ...
        calculateAverageStarRate(resId);
